...
module.exports.deleteCard = (req, res) => {
  ...
  Card.findByIdAndRemove(cardId)
    .then(cards => res.send({ data: cards }))
    .catch(err => {
      if (err.name === 'CastError') return res.status(ERROR_CODE_404).send(errorsMessagee["404del"])
...
.then(cards => res.send({ data: cards }))
.catch(err => {
  if (err.name === 'ValidationError') return res.status(ERROR_CODE_400).send(errorsMessagee["400likes"])
  res.status(500).send(err)})
}
